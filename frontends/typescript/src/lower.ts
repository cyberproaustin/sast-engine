...
export function lowerProgram(opts: LowerOptions): IRDoc {
  ...
  const provenance = classifyModuleProvenance(opts.rootDir, sources);
...
/**
 * Classifies source origin without removing anything from the program.
 *
 * Path conventions settle the categories that are conventions. A nested package needs
 * a second fact: its own package manifest, a licence boundary, or an explicit upstream
 * source marker. That distinction matters in uptime-kuma, whose application and two
 * checked-in packages all sit below `server/modules/`.
 */
function classifyModuleProvenance(rootDir: string, sources: ts.SourceFile[]): Map<string, ModuleProvenance> {
  const workspacePatterns = workspacePatternsAt(rootDir);
  const packageRoots = new Set<string>();
  const upstreamRoots = new Set<string>();
  const rootPackage = readJSON(path.join(rootDir, "package.json"));
  const rootDependencies = new Set<string>([
    ...Object.keys(objectField(rootPackage, "dependencies")),
    ...Object.keys(objectField(rootPackage, "devDependencies")),
    ...Object.keys(objectField(rootPackage, "optionalDependencies")),
  ]);

  for (const sf of sources) {
    let dir = path.dirname(sf.fileName);
    while (dir !== rootDir && isUnder(rootDir, dir)) {
      const rel = moduleIdOf(rootDir, dir);
      const nestedPackage = readJSON(path.join(dir, "package.json"));
      if (
        typeof nestedPackage.name === "string" &&
        typeof nestedPackage.version === "string" &&
        nestedPackage.private !== true &&
        !matchesWorkspace(rel, workspacePatterns)
      ) {
        packageRoots.add(rel);
      }
      if (
        rel.split("/").includes("modules") &&
        LICENCE_FILES.some((name) => fs.existsSync(path.join(dir, name))) &&
        !matchesWorkspace(rel, workspacePatterns)
      ) {
        upstreamRoots.add(rel);
      }
      dir = path.dirname(dir);
    }

    if (moduleIdOf(rootDir, sf.fileName).split("/").includes("modules") && ORIGINAL_SOURCE.test(sf.text)) {
      upstreamRoots.add(moduleIdOf(rootDir, path.dirname(sf.fileName)));
    }

    const parts = moduleIdOf(rootDir, sf.fileName).split("/");
    const modulesAt = parts.lastIndexOf("modules");
    if (modulesAt >= 0 && modulesAt+1 < parts.length && rootDependencies.has(parts[modulesAt+1])) {
      upstreamRoots.add(parts.slice(0, modulesAt+2).join("/"));
    }
  }
  ...
  for (const sf of sources) {
    ...
    if (EXAMPLE_PATH.test(moduleId)) {
      ...
      [...upstreamRoots].some((root) => isWithin(moduleId, root))
    ) {
      origin = "vendored";
    } else if (GENERATED_HEADER.test(sf.text.slice(0, 4096)) || isGeneratedDataTable(sf)) {
      origin = "generated";
    }
...
function isWithin(moduleId: string, directory: string): boolean {
  return moduleId === directory || moduleId.startsWith(`${directory}/`);
}
...
function workspacePatternsAt(rootDir: string): string[] {
  const rootPackage = readJSON(path.join(rootDir, "package.json"));
  const workspaces = rootPackage.workspaces;
  const patterns = Array.isArray(workspaces)
    ? workspaces.filter((item): item is string => typeof item === "string")
    : Object.keys(objectField(rootPackage, "workspaces")).length > 0
      ? ((objectField(rootPackage, "workspaces").packages as string[] | undefined) ?? [])
      : [];

  const lerna = readJSON(path.join(rootDir, "lerna.json"));
  if (Array.isArray(lerna.packages)) {
    patterns.push(...lerna.packages.filter((item): item is string => typeof item === "string"));
  }
  try {
    const yaml = fs.readFileSync(path.join(rootDir, "pnpm-workspace.yaml"), "utf8");
    for (const match of yaml.matchAll(/^\s*-\s*["']?([^"'#]+?)["']?\s*$/gm)) {
      patterns.push(match[1].trim());
    }
  } catch {
    // No workspace declaration is the common case.
  }
  return patterns;
}

function matchesWorkspace(relativeDir: string, patterns: string[]): boolean {
  let matched = false;
  for (const raw of patterns) {
    const negative = raw.startsWith("!");
    const pattern = (negative ? raw.slice(1) : raw).replace(/^\.\//, "").replace(/\/$/, "");
    const escaped = pattern.replace(/[.+?^${}()|[\]\\]/g, "\\$&");
    const regex = new RegExp(`^${escaped.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*")}$`);
    if (regex.test(relativeDir)) matched = !negative;
  }
  return matched;
}

// A generated lookup table is recognizable from its structure even when it carries no
// banner. pdfjs's glyph map is more than four thousand literal assignments to the same
// table and fewer than one line in ten does anything else; a normal source module never
// approaches that shape.
function isGeneratedDataTable(sf: ts.SourceFile): boolean {
  let statements = 0;
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (
      ...
      (ts.isPropertyAccessExpression(node.expression.left) || ts.isElementAccessExpression(node.expression.left)) &&
      isPrimitiveLiteral(node.expression.right)
    ) {
      literalAssignments++;
    }
    ts.forEachChild(node, visit);
  ...
  visit(sf);
  return literalAssignments >= 200 && literalAssignments * 10 >= statements * 9;
}
