...
var claims = map[string]Claim{
	...
	"CWE-639": {State: Partial, Reason: "five rules, and the second exists because the first had to presume something. `unowned-record-access` reports a record selector chosen by the caller with no relation to the caller's identity, and it accepts a call carrying that identity as the relation -- which is right for an assertion helper and wrong for a permission check that names a scope. `authorization-scoped-elsewhere` states the relation instead: when an enforced check is scoped to one key and a store write the check admitted is keyed by a different one, the analysis looks for something tying them -- a lookup carrying both, a comparison between them, a comparison against the actor, or a selection that carries both -- and reports when there is none. Existence does not count: `featureExists(id)` proves the row is there and mentions the authorized key not at all. Measured over ten production repositories it produced two findings and both were true, at unleash's segment controller and umami's report endpoint. What it does not catch is stated: an authorization that lives in route middleware rather than in the handler, because the relation is judged inside one function and a middleware gate is not in it; a write in a service the handler calls, for the same reason; a key spelled as a NAME rather than an id, which loses unleash's `featureName` because `name` is what most payload fields are called; a key spelled as a SLUG, tried and withdrawn after umami's link and pixel rename endpoints -- whose purpose is to set the row's own slug -- were two of four findings; and a CREATE, whose identifiers are the new row's own fields rather than a selector reaching an existing one. Judgements on entry points the framework handed no identity are set aside rather than reported: 42 of those were adjudicated by hand against sixteen production repositories at 0.00 precision, and setting them aside cost nothing on the vulnerable corpus. One further limit is a measurement rather than an argument: a program having an identity source SOMEWHERE is not evidence that a given handler can reach it. Recognising a verified JWT as an identity turned n8n from honestly not-evaluated into 181 findings, and the ones examined were invitation and password-reset routes that are unauthenticated by design and have no owner to check against -- six true positives in the vulnerable corpus did not pay for that, so the analysis stays quiet where the identity it needs is not visible from the handler. `authorization-through-a-different-accessor` is the third, and it exists because the second one still needs the caller's scope to have arrived in the REQUEST. Very often it did not: the authentication layer established it, the handler holds it inside a context object, and the question the handler asks is `may I act here` rather than `may I act on THIS`. What that leaves comparable is the ACCESSOR -- medplum's rotate-secret handler asks `ctx.repo.isProjectAdmin()` and then reads and rotates the client application through `ctx.systemRepo`, a sibling accessor of the same context constructed with `superAdmin: true`, keyed by the route's own id and with nothing relating that record to the caller's project. The rule states the swap and nothing about privilege: it does not know which of the two accessors is the elevated one, and a handler that asks and acts through the same accessor is silent whichever one it chose. Measured over the same ten production repositories it produced ONE finding, medplum's, which an independent review and a source adjudication had already confirmed, and no others -- and the same measurement is what let its operation list include reads as well as writes, which the rule above cannot afford: widening it added no finding the write words had not already produced. What it does not catch is stated. A check whose receiver is not a property of anything -- a bare `requireAdmin()` -- names no accessor and there is nothing to compare. A check written as a decorator or a middleware is not in the handler's body. An accessor reached through a named helper rather than through a callback the handler wrote inline is not followed, for the reason the key relation gives: deciding which caller's gate governs which callee's operation is a judgement this analysis declines. And the check has to be RECOGNISABLE as an authorization from its name, because with no identity value handed to it there is nothing else to go on -- a permission predicate named in a vocabulary this list does not hold reads as an ordinary guard and the handler is passed over" +
		...
		By: []string{"unowned-record-access", "authorization-scoped-elsewhere",
			...
			"record-named-under-no-declared-permission"}},
	...
	"CWE-352": {State: Partial, Reason: "a state-changing entry point missing the anti-CSRF control most of its comparable peers apply, plus `state-change-without-csrf` for two handler-local facts the population cannot see: a Django handler explicitly removes CSRF enforcement and then reaches an ORM/session write through one resolved helper, or an unrestricted Django GET/ANY handler starts such a write from its entry block. The framework boundary matters: applying the safe-method spelling to every framework produced three false findings in the fixture corpora, because a bearer-authenticated API does not need a cookie-bound token. Cache refresh and transitive initialization writes are excluded by measurement; neither is the request state an attacker is making the application change. The original peer rule remains inferred and non-gating, and measured at zero across both corpora because application-wide middleware distinguishes none of its entry points. The handler-local rule is asserted from the declaration and graph rather than inferred from peers",
		By: []string{"expectations", "state-change-without-csrf"}},
	...
	"CWE-306": {State: Partial, Reason: "an entry point missing an authentication control most of its comparable peers on the same mount apply. Inferred from the population and therefore informing rather than gating (ADR-010). The mount is part of the population because registering a route on a deliberately public router is an author decision: medplum's public FHIR router produced the only two findings across twenty repositories when it was compared with a separate protected router in the same file, and both were false. " +
		"The inference is WITHHELD, and the withdrawal printed with the line that caused it, where the entry point authenticates the other way an application can: the caller presents a value the program itself names a secret -- the caller-credential vocabulary, `token`, `secret`, `apiKey`, `password` -- and the handler, or a function one hop below it, hands that value to a record selection. That is a signing link, a password-reset link, an email-verification link, an unsubscribe link or a webhook callback, and every one of them mounts nothing, which is precisely what a comparison against session middleware reads as an open door. Measured: enumerating documenso's tRPC surface took it from 62 entry points to 355 and produced five CWE-306 findings, and an independent reader judged all five false against the source; the same evidence marks 15 of documenso's 349 routes, and reading every one found a signing token, an organisation invite token, a team-email verification token or a direct-template token in each. " +
		"What separates that from an insecure direct object reference is the only thing available: whether the caller could have GUESSED the value. The program answers it in the name it gave the field, so `documentId` and `fieldId` select a record and are not authentication, and a leaf ending in `id` is refused outright because `tokenId` and `apiKeyId` are primary keys wearing a credential word. The direction of every doubt is to keep reporting. " +
		"Four things it does not do, stated rather than implied. It does not prove the program REFUSES when the selection finds nothing: for four of documenso's five that is a library call's own failure mode one module away, and requiring control-flow dominance across the call boundary would lose all four. It stops one hop below the handler, the same bound the authentication fact uses, so a lookup two frames down is invisible. It binds an argument to a parameter by NAME only -- an options-object key, a keyword argument -- because a POSITION does not survive a receiver: Python declares `self` and `cls` as parameter zero at a call site that writes neither, and bound by position saleor's `setPassword` came out citing `User.objects.get(email=email)` as a selection keyed by the caller's password, when the row is selected by the email and the token is checked two statements later. That route does authenticate by token; the sentence the engine wrote about it was false, so a secret handed down positionally is now a stated miss and `testdata/credential-instead-of-session` asserts it. And it only answers an expectation whose missing control the engine could CLASSIFY as authentication -- a population built on middleware with a name the model does not recognise reports CWE-284, which is the honest number for `something is missing and I cannot say what`, and evidence about authentication does not refute it. " +
		"The withdrawal is confined to THIS question, and the same shape stays a finding under the other one: `unowned-record-access` reported healthchecks' email-verification token -- fixed upstream in 6b634df -- because the token did not bind the address it verified, and nothing here touches that. One asks whether the caller was identified at all; the other asks whether the key covers what it admits, and a token that authenticates perfectly can still be the wrong key. " +
		"It still cannot distinguish an endpoint unauthenticated by design from one that forgot inside one shared mount, which is what a declaration supplies",
		By: []string{"expectations"}},
	"CWE-862": {State: Partial, Reason: "three rules, and all three say the same thing about a control that is absent: the program performs it somewhere comparable and not here. " +
		...
		"`permission-selected-and-never-checked` is the fourth, and it is the only one that needs no comparison at all -- no peer, no sibling, no second path. The control is COMPUTED and then dropped. saleor's `check_metadata_permissions` decodes the object's type out of the global id the caller sent, looks the required permission up in the map that exists for exactly this, rejects a type the map does not cover, and returns; nothing ever asks whether the caller HOLDS what it found. Every line is individually right, which is why the rules above and the four analyses beneath them all pass over it: the lookup is correct, the rejection is real, and the branch it sits on is obeyed. What is wrong is that the function names itself a permission check and the permission is dead. " +
		"The rule is stated over the program's own words, the way `discarded-restriction` is, because an engine cannot know what a value is FOR. Three names have to agree: the producer -- the mapping or call the value came out of -- the name the value was bound to, and the enclosing function, which has to be named for enforcing rather than for fetching, since `get_permissions` returns permissions as data and is not this. An existence test is deliberately NOT a use, and that is the finding rather than a concession: asking whether a lookup returned anything is a question about the mapping and not about the caller. Stating it that way also keeps the rule off a frontend accident -- Python lowers `if private:` as a truthiness comparison and lowers nothing at all for `if not meta_permission:`, so a rule that read 'no comparison mentions it' would go silent the day that asymmetry is fixed. " +
		"Measured over the ten production repositories the engine is scored on: the shape occurs once, at saleor `graphql/core/mutations.py:579`, reported under checkoutComplete, and an independent review and a source adjudication had already confirmed it. What it does not catch is stated. A permission handed to something that merely records it counts as consulted, because a call is a use and this rule does not read what the callee does with what it was given. A permission compared against a value that is not a literal nothing counts as consulted, for the same reason -- `if perm == MANAGE_ORDERS` is a real question even where the answer is thrown away. A control that is not a PERMISSION in the program's own vocabulary is invisible: a rate-limit budget looked up and dropped has the same shape and none of the words. And a function that returns the permission it found is outside this rule by construction, however little its callers do with it, because the question then belongs to the caller",
		By: []string{"expectations", "sibling-guard-differential", "control-omitted-on-sibling-path",
			"permission-selected-and-never-checked"}},
	...
	"CWE-770": {State: Partial, Reason: "six rules, and what they have in common is that the program itself supplies the evidence. `expectations` reports an entry point missing a throttle most of its comparable peers apply. " +
		...
		"What they do not catch is stated. A basic block is the whole evidence for the lookup question, so a write inside a LOOP BODY or a switch arm -- where neither frontend states a position, deliberately -- is passed over rather than guessed at. A container IMPORTED from another module has no value in this IR, so the write into it records no destination and nothing is judged; the container has to be declared in the module the write is in, which is where uptime-kuma's is. A cap is only recognised where the extent is measured against a number written down: `cache.size > MAX_ENTRIES` through a constant is not matched and will suppress nothing, and the bias there costs recall rather than precision. A key that arrives through a call the frontend could not resolve carries no classification and is not a key the caller chose as far as this rule can tell -- linkwarden's bulk endpoints are invisible for exactly that reason, because its `@/...` path aliases resolve to no module. " +
		"Three confirmed consumption shapes remain distinct rather than being forced into the read rule. A caller-supplied SLICE BOUND is absent from the IR: linkding's `query_set[:int(limit)]` lowers to an undifferentiated `[index]` property with no value for the bound, so no existing analysis can state what amount was chosen. mitmproxy's four decompressor calls and `codecs.decode` are present, but the proxy transport that supplied their bytes is not a source in the model; a call-only decompressor rule was declined because every ordinary decompression would match. And archivebox's `IMG_SRC_ATTR_RE.sub` carries only its text argument in Call.Args: the callback argument is absent, and the IR has no fact saying a substitution invokes that callback once per match, so the multiplication by recursive traversal cannot be joined. " +
		"The broad repetition alternative was measured and declined: a loop with a bound carrying untrusted-input or upstream-response encloses 841 call/class matches at 606 unique call sites across the ten repositories before asking what any call costs. That population is ordinary iteration, not evidence of denial of service; the ITERATION half remains not built under CWE-834",
		By: []string{"expectations", "unbounded-retention-by-caller-key", "remote-input-read-without-size", "refusal-inside-a-repeating-callback", "expensive-entry-outside-rate-limiter", "rate-limit-key-from-trusted-forwarding-header", "rate-limit-key-from-a-client-supplied-header"}},
	...
	"CWE-200": {State: Partial, Reason: "information exposed to somebody who should not have it. Most decidable forms remain claimed under the number that names the DATA -- credentials under CWE-522, failure detail under CWE-209, the process environment under CWE-497, personal information under CWE-359. `credential-headers-forwarded-cross-host` asserts one parent-level form because the program supplies every part of the audience judgement: it copies the complete inbound Headers collection, replaces the outbound URL authority, and forwards the collection without dominating removals of both Cookie and Authorization. Same-host forwarding and a collection with both credential fields removed are measured negatives. The earlier X-Powered-By rule remains withdrawn: it measured three of four vulnerable Express applications against four of ten across twenty-eight production repositories and put a finding into every one of 55 Express fixtures, for a header routinely stripped by a reverse proxy",
		By: []string{"credential-headers-forwarded-cross-host"}},
	...
	"CWE-863": {State: Partial, Reason: "a check that is skipped by omitting what it checks. `verification-skipped-by-absent-operand` reports a refusal conditioned on the PRESENCE of the two values it compares -- `if (claim && expected && claim !== expected) throw` -- where the party who supplies one of them decides whether the check runs, by leaving it out. documenso's embedded-signing presign verifier is the measured case: it takes `scope` as an optional parameter, and two file routes call it with no scope, so a token minted for one purpose is accepted for every purpose those routes serve. Nothing is absent from the program and the check reads as correct in the file. Three facts, each of them a number measured over ten production repositories: the conjunction shape occurs SEVEN times; requiring the truthy side to leave by THROWING leaves two; requiring the two operands to have been written with the same trailing name leaves ONE, and it is documenso's. The five that naming removes are difference tests -- a rename that needs both paths, a font weight against a document's modal font, a checkout address against the previous one -- and every one of them wants its presence tests. What it does not catch is stated. A presence test spelled at a DIFFERENT name than the value it guards -- `scope` against `expectedScope` -- is a miss, because the containment test that would admit it also admits `fontRef` inside `modalFontRef` and `address` inside `old_address`, which are two of the five. A refusal written as a response rather than a throw -- `return c.json({error}, 403)` -- is a miss, because a `return` is also how an ordinary predicate answers and the corpus contains far more of the second. A disjunction is not read at all: `if (!a || !b || a !== b) throw` refuses when either is missing and is the correct way to write this. And the same hole written across two functions, where the presence test guards the CALL to a verifier rather than the comparison inside it, is not in view: the judgement is made inside one function",
		By: []string{"verification-skipped-by-absent-operand"}},
	...
	"CWE-613": {State: Partial, Reason: "a credential cookie whose lifetime is written into the call and is longer than a month, and a jsonwebtoken.sign call where every possible home of an expiry is enumerated and neither `exp` nor `expiresIn` is present. An inline payload makes an omitted options argument knowable; a payload built elsewhere is silence, because its claims are not visible. Test-only issuance is excluded: a token a test creates for itself has no deployment lifetime. Only cookies whose NAME says they carry a credential are asked about, because a year-long theme preference is a feature. Thresholds are per-rule rather than per-keyword, because express counts milliseconds and Flask counts seconds under names that look the same. A lifetime computed at runtime, a server-side session store expiry, and a verifier's missing maxAge when the issuer cannot be paired are not decidable here. Measured across ten production repositories the newly knowable omitted-options shape added one finding, uptime-kuma's authentication token; its inline payload has no exp and the corresponding verify supplies no maxAge. measured across ten production repositories the newly knowable omitted-options shape added one finding, uptime-kuma's authentication token; its inline payload has no exp and the corresponding verify supplies no maxAge. `credential-not-reissued-with-subject` is the third and it asks a different question about the same subject: not how long a credential lasts, but whether it still covers what it admits. One store call that says BOTH what to write when a record is NEW and what to write when it already EXISTS -- an upsert's `create` and `update`, Mongo's `$setOnInsert` and `$set` -- where the new-record half mints a field whose name says it is a credential and the existing-record half rewrites the address that credential was issued for without reissuing it. This is the healthchecks weakness stated as a write: there the token was seeded with the channel's immutable id and the server secret so it could not depend on the address, here the token is random and the same independence comes of not rotating it, and the remedy is the same sentence. What makes the absence decidable is that the program answers it -- the create half is this application's own statement of what a new record of this kind needs, so nothing outside the call is read and no population is compared against. Measured over ten production repositories it produces TWO findings, documenso's `set-document-recipients.ts:155` and `set-template-recipients.ts:136`, and both are the weakness. The broader shape it was reshaped from, a create somewhere in the program naming a credential and an update elsewhere not naming it, matches 173 calls over the same corpus and was not built. What that costs is stated: an update written as its own call is not reported, which loses documenso's `update-envelope-recipients.ts:142` -- an adjudicated true finding -- and its `admin/update-recipient.ts:22`, because `update` alone is not a store-write verb this model recognises and admitting it produced two more findings that were not the weakness, a signing completion recording the signer's own address and a template placeholder whose new address is a constant. A call whose options were assembled elsewhere is silence rather than absence, and a credential rotated on a LATER line of the same handler is not looked for",
		By: []string{"long-lived-session", "unexpiring-token", "credential-not-reissued-with-subject"}},
	"CWE-1022": {State: Partial, Reason: "a window opened onto a named target with no third argument, which is where noopener would be -- the opened page keeps a live reference back through window.opener and can navigate the page behind it. A statically rooted application path and a blob URL the page created itself cannot hand that reference to another origin and are excluded; a //host network-path reference and the browser-equivalent /\\host spelling are not application paths and remain reported. A slash followed immediately by an unproven template span is not treated as local, because the span can supply the second slash. Only the scripted form: markup written as a template string is text to this engine and its attributes are not read",
