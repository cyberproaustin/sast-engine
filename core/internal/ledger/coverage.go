...
var claims = map[string]Claim{
	...
	"CWE-698": {State: Asserted, Reason: "a rejection the program wrote and then walked past. The engine's only judgement about the SHAPE OF THE GRAPH: an error status is written into the response and the work it was refusing is still unavoidable, so the refusal is a status code and nothing more. It needs no declared expectation, which is why it is worth having -- the program has already said in its own code that the request should not proceed. Three shapes that look identical are excluded because they are correct: a branch whose other side does the work reconverges on nothing, a helper the language declares `never` does not come back, and a catch handler runs INSTEAD of the rest rather than after it. A redirect is deliberately not a rejection: `res.redirect(next)` after a successful login is the same call as after a failed one, and one production repository writes eight of the first kind in a single function",
		By: []string{"rejection-without-return"}},
	...
	"CWE-798": {State: Partial, Reason: "two shapes, both matched on having been written down rather than on what the value says: a key argument passed as a literal to a signing or session call, and a literal assigned to a configuration key whose name holds the word secret, password or key. A key read from the environment or a vault is not a literal and never matches. The second shape is matched on a WORD in the key because configuration keys are compound, and it excludes three literal shapes measured on the clean corpus that a key-named setting holds when it is NOT holding a key: an endpoint the credential is sent to, a sentence from a settings schema, and the mask a value is replaced with before it is logged. The word token is deliberately not in the list -- across twenty-eight repositories every configuration key holding it held a URL, a header name or a form field. A third shape needs no call and no destination at all: a value whose own SHAPE is a credential -- a password in a connection string, a signed token, and the six provider-issued identifiers whose format their issuer defined. Those six measured zero outside test files across the whole clean corpus, which is what a rule with no room to guess looks like; the other two measured six, every one of them a real credential in a real repository",
	...
	"CWE-770": {State: Partial, Reason: "three rules, and what they have in common is that the program itself supplies the evidence. `expectations` reports an entry point missing a throttle most of its comparable peers apply. " +
		...
		"Measured over ten production repositories the two produced two findings and both were true. " +
		...
		By: []string{"expectations", "unbounded-retention-by-caller-key", "refusal-inside-a-repeating-callback"}},
