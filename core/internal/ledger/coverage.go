...
var claims = map[string]Claim{
	...
	"CWE-770": {State: Partial, Reason: "six rules, and what they have in common is that the program itself supplies the evidence. `expectations` reports an entry point missing a throttle most of its comparable peers apply. " +
		...
		"`remote-input-read-without-size` asks what a read call was handed as well as what it omitted: the receiver must already carry remote untrusted-input, the call must omit its positional size, and the source must be anchored to an enumerated remote entry point. The unqualified shape occurs twice across the ten production repositories -- linkding's HTTP import and a management-command import -- and the trust condition removes the command; registering DRF's custom action exposes the second confirmed HTTP upload. A read with a positional size is silent. " +
		...
		By: []string{"expectations", "unbounded-retention-by-caller-key", "remote-input-read-without-size", "refusal-inside-a-repeating-callback", "expensive-entry-outside-rate-limiter", "rate-limit-key-from-trusted-forwarding-header"}},
