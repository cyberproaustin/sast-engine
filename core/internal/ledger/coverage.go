...
var claims = map[string]Claim{
	...
	"CWE-95": {State: Asserted, Reason: "untrusted data reaching a language evaluator: eval, Function, Python exec",
		By: []string{"untrusted-to-interpreter"}},
	"CWE-89": {State: Partial, Reason: "untrusted data COMPOSED into the statement argument of a described SQL API, where composed means composed into the value THIS sink receives rather than anywhere in its history. A parameterized call cannot match, because the channel names only the interpreted argument. Known imprecision: `execute` is also what a CQRS bus and a SQLAlchemy Session are called on, and neither takes a statement string -- telling them apart needs the receiver's type, which one frontend supplies patchily and the other not at all, so those report at reduced confidence rather than being excluded A recall audit named one shape this cannot reach: pg-promise calls its query methods `one`, `many`, `any` and `none`, which are 2020 call sites across the clean corpus and almost none of them a query -- and the receiver that would tell them apart is the result of calling the result of a require, a callee the frontend does not resolve. A pg-promise application is analysed for everything else and silent about its SQL",
		By: []string{"untrusted-to-interpreter"}},
	...
	"CWE-798": {State: Partial, Reason: "two shapes, both matched on having been written down rather than on what the value says: a key argument passed as a literal to a signing or session call, and a literal assigned to a configuration key whose name holds the word secret, password or key. A key read from the environment or a vault is not a literal and never matches. The second shape is matched on a WORD in the key because configuration keys are compound, and it excludes three literal shapes measured on the clean corpus that a key-named setting holds when it is NOT holding a key: an endpoint the credential is sent to, a sentence from a settings schema, and the mask a value is replaced with before it is logged. The word token is deliberately not in the list -- across sixteen repositories every configuration key holding it held a URL, a header name or a form field. A third shape needs no call and no destination at all: a value whose own SHAPE is a credential -- a password in a connection string, a signed token, and the six provider-issued identifiers whose format their issuer defined. Those six measured zero outside test files across the whole clean corpus, which is what a rule with no room to guess looks like; the other two measured six, every one of them a real credential in a real repository",
		By: []string{"hardcoded-secret", "credential-in-url", "signed-token", "aws-key-id", "github-token", "slack-token", "stripe-key", "google-api-key", "npm-token"}},
